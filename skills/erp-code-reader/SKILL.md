...
4. **提表名并解语义(闭环)**:`grep -oE '\b[a-z][a-z0-9]{1,6}_t\b' <文件> | sort | uniq -c | sort -rn`,把前几个表名**一次性**交给 `tdict r.t --brief "a_t,b_t,c_t"` 批量解码(**不加 `--brief` 会打出每张表的全部字段,7 张表就 700 行**);要看某张表的字段时再单表 `tdict r.t <表名>`。
   - ⚠️ `type_t` 是 4GL 类型记录、**不是表**;`ds.xxx_t` 这种 schema 限定写法朴素正则会漏;拼在字符串里的表名也会漏。
   - ⚠️⚠️ **grep 出 0 张表 ≠ 这个程序不碰表**。T100 常见"主程序 + 应用元件"结构:明细逻辑在 `com/sub/4gl/s_<程序>.4gl`(元件)或 `com/lib/**` 里,主文件只剩排程/画面骨架,SQL 还可能是动态拼的。实测 `erp/apc/4gl/apcp300.4gl`(691 行)grep 出 **0 张真实表**,它的 `inde_t(64)/indf_t(36)/pcca_t(13)` 全在 `com/sub/4gl/s_apcp300.4gl`。此时先 `find <源码树> -iname "*<程序编号>*"` 找同编号的元件/子程序文件再 grep。**注意元件文件名是 `s_apcp300.4gl`,代码里 `s_apcp300_send` 只是它里面的函数名**。
