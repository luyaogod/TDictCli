...
1. **先问工具"这是什么"**:`tdict prog <程序编号>`给出中文作业名、程序类别、归属模块、以及**哪些作业用了它**;反向从业务词找程序用 `tdict prog --kw <业务词>`。**其次**才看文件头 `#+ Description:`(没有作业登记的文件如 `com/inc/**/*_rep.4gl`、`q_*_wf.4gl` 只能看文件头)。
   - ⚠️ **先确认这一族数据在本地可用**:`tdict db status` 里「程序与作业」显示 `✗` 时,`prog` 会整体报错——命令加 `--conn <环境名>` 免写盘直查(`tdict env` 看环境名),或先 `tdict db sync`。
   - ⚠️ **`prog` 只登记主程序**:`aapq110_01` 这类子程序要降级查主程序 `aapq110`,而**二者用途常常不同**(主程序"供应商对账单明细查询" vs 子程序"报表列印")——子程序自己做什么只能看它的文件头。
   - ⚠️ **`prog` 不给"这个程序用什么表/开窗/校验"**,那要靠下面第 4、5 步。
...
>
> - **`com/lib/**`、`com/sub/**` 的库与元件在 `tdict prog` 里没有条目**,工具也没有"这个 .4gl 库做什么"的命令。
