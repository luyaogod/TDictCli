...
// tdict msg:查询系统消息档(gzze_t,由作业 azzi920 维护;运行时 cl_err/cl_getmsg
// 按编号+语言取用)。AI 在代码里遇到消息编号(如 std-00006 / azz-00041 / -263)
// 时用它查文本、建议处理方式与技术细节。
//
// 数据源与 rt/rv 等一致(本地 SQLite 镜像 / --conn <环境> 远程直查);
...
语言: 默认只显示 --lang(缺省 zh_CN)那一行;该编号无此语言时列出可用语言。`,
  ...
  tdict msg -- -100                     # 负整数编号(SQLCODE)需用 -- 分隔
  tdict msg aoo-00120 --conn 主机正式区   # 远程直查该环境`,
