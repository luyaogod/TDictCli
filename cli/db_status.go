...
查询命令报「本地库尚未包含 XXX 数据」时用它定位;补齐用 tdict db sync。
看的是 -d/--db 或 TDICT_DB 指向的库(与查询命令同一份);库不存在只报状态、不报错。
只读,不改任何文件;也不需要 config.json(还没配环境也能看)。`,
	Example: `  tdict db status
  tdict db status -d D:\data\erp_data.db
  tdict db status --json`,
	Args: cobra.NoArgs,
...
func printLocalStatus(st *dbsync.LocalStatus) {
	if !st.Exists {
		fmt.Printf("本地库不存在: %s\n", st.Path)
		fmt.Println("建立本地库: tdict db sync(需能连 ERP);或查询时用 --conn <环境名> 远程直查,不必同步。")
		return
	}
