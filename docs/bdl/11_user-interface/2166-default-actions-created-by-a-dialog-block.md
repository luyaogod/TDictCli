...
source: "fgl-topics/c_fgl_DIALOG_block_default_actions_2.html"
breadcrumb: "User interface > Dialog instructions > Declarative dialogs (DIALOG - at module level) > Using declarative dialogs > Default actions created by a DIALOG block"
type: "concept"
---
...
[Table 1](2094-default-actions-created-by-a-dialog-block.md) lists the default actions created for the `DIALOG` interactive instruction, for
...
| `nextrow` | Moves to the next row in a list displayed in one row of fields.See [note (1)](2094-default-actions-created-by-a-dialog-block.md). |
| `prevrow` | Moves to the previous row in a list displayed in one row of fields.See [note (1)](2094-default-actions-created-by-a-dialog-block.md). |
| `firstrow` | Moves to the first row in a list displayed in one row of fields.See [note (1)](2094-default-actions-created-by-a-dialog-block.md). |
| `lastrow` | Moves to the last row in a list displayed in one row of fields.See [note (1)](2094-default-actions-created-by-a-dialog-block.md). |
| `find` | Opens the fglfind dialog window to let the user enter a search value, and seeks to the row matching the value.See [note (2)](2094-default-actions-created-by-a-dialog-block.md). |
| `findnext` | Seeks to the next row matching the value entered during the fglfind dialog.See [note (2)](2094-default-actions-created-by-a-dialog-block.md). |
