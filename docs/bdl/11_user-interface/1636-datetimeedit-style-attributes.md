---
title: "DateTimeEdit style attributes"
source: "fgl-topics/r_fgl_presentation_styles_datetimeedit_style_attributes.html"
breadcrumb: "User interface > Form definitions > Presentation styles > Style attributes reference > DateTimeEdit style attributes"
type: "reference"
---
...
Defines the icon name to use for the button that opens the date/time picker.
...
When set to `"modal"`, the calendar is intrusive to the `DATETIMEEDIT`
...
When set to `"dropdown"`, the calendar is less intrusive: The user can type directly
into the `DATETIMEEDIT` field. A single click on a date validates and closes the calendar.
There is no way to cancel the selected date.
