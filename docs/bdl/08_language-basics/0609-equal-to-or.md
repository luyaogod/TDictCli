---
title: "Equal to (== or =)"
source: "fgl-topics/c_fgl_operators_EQUAL.html"
breadcrumb: "Language basics > Operators > List of expression elements > Comparison operators > Equal to (== or =)"
type: "concept"
---
...
A single `=` equal sign can be used as an alias for the `==`
operator. However, consider using a double `==`, to distinguish comparisons from
...
`FALSE`. When two corresponding members are `NULL`, they are
considered as equal. This second syntax allows you to compare all members of records, but records
must have the same structure. Pay attention to array and `TEXT/BYTE` record members.
For more details, see [Comparing records](0722-comparing-records.md "Records can be compared with the == comparison operator and the .* notation.").
