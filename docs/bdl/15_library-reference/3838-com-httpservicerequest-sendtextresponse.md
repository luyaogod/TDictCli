---
title: "com.HttpServiceRequest.sendTextResponse"
source: "fgl-topics/c_gws_ComHTTPServiceRequest_sendTextResponse.html"
breadcrumb: "Library reference > Extension packages > The com package > Web services classes > The HttpServiceRequest class > HttpServiceRequest methods > com.HttpServiceRequest.sendTextResponse"
type: "concept"
---
...
3. txt defines the string containing the
   data to be sent.

## Usage

The `sendTextResponse()` method performs the HTTP response by sending the status
(code) and description (description), followed by the headers
previously set, and text data contained in the string as body.
...
Automatic conversion from locale to user-defined charset is performed when possible, otherwise an
exception is thrown.
