---
title: "com.HttpServiceRequest.sendDataResponse"
source: "fgl-topics/c_gws_ComHTTPServiceRequest_sendDataResponse.html"
breadcrumb: "Library reference > Extension packages > The com package > Web services classes > The HttpServiceRequest class > HttpServiceRequest methods > com.HttpServiceRequest.sendDataResponse"
type: "concept"
---

# com.HttpServiceRequest.sendDataResponse

> Sends an HTTP response with data of a BYTE variable.
...
3. b defines a
   `BYTE` type containing the data to be sent.

## Usage

The `sendDataResponse()` method performs the HTTP response by sending the status
(code) and description (description), followed
by the headers previously set and binary data contained in the `b`
program variable as body.
...
The b parameter is defined as a `BYTE` and must be located in
memory and not `NULL`, otherwise the operation fails.
