...
source: "fgl-topics/c_fgl_odiagpgs_023.html"
breadcrumb: "SQL support > SQL database guides > PostgreSQL > Data manipulation > Temporary tables"
type: "concept"
...
PostgreSQL supports temporary table creation statements similar to Informix:
...
There is a
little syntax difference in the `SELECT INTO TEMP` instruction. PostgreSQL uses the
`INTO TEMP` clause after the `SELECT`
list:

```
SELECT * INTO TEMP temptab FROM source
```
