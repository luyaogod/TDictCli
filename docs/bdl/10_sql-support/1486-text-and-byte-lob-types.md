...
source: "fgl-topics/c_fgl_odiagsqt_021.html"
breadcrumb: "SQL support > SQL database guides > SQLite > Data dictionary > TEXT and BYTE (LOB) types"
type: "concept"
...
SQLite 3 provides the `TEXT` and `BLOB` native data types for large
objects storage.

## Solution

The SQLite database interface can convert BDL `TEXT` data to SQLite
`TEXT` and `BYTE` data to SQLite `BLOB`.
