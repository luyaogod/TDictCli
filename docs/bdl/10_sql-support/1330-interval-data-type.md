...
source: "fgl-topics/c_fgl_odiagmys_030.html"
breadcrumb: "SQL support > SQL database guides > Oracle® MySQL / MariaDB > Data dictionary > INTERVAL data type"
type: "concept"
...
MySQL and MariadDB support an `INTERVAL` data type. However, the interval values
are used mainly for date/time calculations
like:
...
`INTERVAL` values can be stored and retrieved from the database. However, interval
