...
查询**系统消息档**（`gzze_t`，由作业 azzi920 维护，运行时 `cl_err`/`cl_getmsg` 按编号取用）。消息编号形如 `std-00001`、`azz-00041`、`lib-xxxxx`，负整数（SQLCODE 如 `-100`）需用 `--` 分隔：`tdict msg -- -100`。
...
- **打包根(topDir)解析**:环境显式 `topDir` > 区域静态表 > SSH 登录探测(如站点 zone "1"/"2" 未显式配置时必须探测;失败会报错提示补 topDir)。
  ...
  "debug": {
    "sshs": [
      { "name": "正式区", "host": "10.0.0.1", "port": 22, "user": "youruser",
        "password": "youruser", "zone": "36", "topent": "99", "topDir": "/u1/topprd",
        "db": {
          "type": "oracle",
          "host": "10.0.0.1", "port": 1521,
          "service": "t35prd",
          "accounts": [
            { "account": "your_schema", "password": "your_schema" },
