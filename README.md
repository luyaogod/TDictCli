...
# 使用指定连接与数据库
tdict db sync --conn 站点your_schema -d D:/path/to/erp_data.db
```
...
{
  "connections": [
    { "name": "站点your_schema", "type": "kingbase", "host": "10.0.0.2", "port": 54321,
      "database": "topprd", "user": "your_schema", "password": "your_schema", "isDefault": true },
    { "name": "惟清your_schema", "type": "oracle", "user": "your_schema", "password": "your_schema",
...
{ "name": "站点your_schema", "type": "kingbase", "host": "10.0.0.2", "port": 54321,
  "database": "topprd", "user": "your_schema", "password": "your_schema", "isDefault": true,
  "source": "manual",
  "viaSsh": { "host": "10.0.0.2", "port": 22, "user": "youruser", "password": "youruser",
...
tdict rt dzea_t --online --conn 站点your_schema      # 指定连接
tdict db ping --conn 站点your_schema                 # 验证连接可达(只读;支持 viaSsh)
tdict db list                                   # 列出连接(含 source/viaSsh)
tdict db discover --env 正式区 --type kingbase --save   # SSH 自动发现并保存连接
```
