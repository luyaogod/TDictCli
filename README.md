...
| 子程序与元件 | 子程序 / 应用元件 / 报表元件 / 组件 / WebService 的登记与中文说明（与主程序登记**互补**，实测交集 0） | 4,036 个（子程序 1,316、应用元件 1,491、报表元件 1,130、组件 95、WS 4） | `tdict prog <编号>` / `tdict prog --sub` |
...
**子程序 / 元件 / 库的登记**（数据来自 `gzde_t` 子程序及应用元件基本数据表，参考作业 azzi901「子程式及元件基本資料設定作業」）——与主程序登记**互补且不重叠**（实测 `gzza_t` 4,147 个主程序、`gzde_t` 4,036 个子程序/元件，交集 0），所以 `tdict prog <编号>` 对两类编号都能答：

```
$ tdict prog aapq110_01
=== aapq110_01 ===
说明: 供应商对账单明细查询报表打印
规格类别: S(子程序)
归属模块: AAP
...
### `tdict db status` — 本地数据覆盖情况

`tdict` 的各查询命令依赖不同**数据族**（表字典/校验带值/系统分类码/字段画面规格/可复用开窗/系统消息/参数定义/程序与作业）。命令报「本地库尚未包含 XXX 数据」时，用它一次看清哪个族没同步、各多少行、缺哪些表：

```
$ tdict db status
本地库: D:\APPS\tdict-portable\erp_data.db  (56.5 MB, 2026-08-26 13:18)
