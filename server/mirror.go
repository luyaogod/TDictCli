...
// mirrorDirFromRoot 读 config.json 顶层 mirror.dir(相对路径转绝对)。
func mirrorDirFromRoot(root map[string]any) string {
	m, _ := root["mirror"].(map[string]any)
	if m == nil {
		return ""
	}
	d, _ := m["dir"].(string)
...
// hMirrorPut 保存镜像根目录(顶层 mirror.dir;其余键保留;目录由拉取时创建)。
func (s *Server) hMirrorPut(w http.ResponseWriter, r *http.Request) {
	...
	root["mirror"] = map[string]any{"dir": abs}
...
func (s *Server) hMirrorPull(w http.ResponseWriter, r *http.Request) {
	...
	hosts, err := host.LoadHosts(s.cfgPath)
	if err != nil {
		fail(w, 500, err)
		return
	}
