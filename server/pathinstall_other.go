...
// getInstallStatus 非 Windows 不自动改 PATH,给出等价的手动命令。
func getInstallStatus() pathInstallStatus {
	exe := exePath()
	dir := filepath.Dir(exe)
	return pathInstallStatus{
		Supported: false,
		ExePath:   exe,
		ExeDir:    dir,
...
func addExeDirToUserPath() (pathInstallStatus, error) {
	return getInstallStatus(), fmt.Errorf("当前系统不支持自动写入 PATH,请按提示手动添加")
}

func removeExeDirFromUserPath() (pathInstallStatus, error) {
	return getInstallStatus(), fmt.Errorf("当前系统不支持自动写入 PATH")
}
